...
**Question 1** : Selon vous, à quoi correspond le champs indiqué par
                *PTTY*?
C'est un port virtuel créé par *gps*
...
Il n'y a pas de gestionnaire de signaux, un *ctrl+C* ne permet pas de fermer le descripteur proprement. 
...
L'heure est dans la trame GPGLL, champs 5.
...
Ouverture: *int fd = open(port, O_RDWR | O_NOCTTY);*
Ecoute: *select(fd+1, &fdset, NULL, NULL, NULL);*
Lecture: *int bytes = read (fd, buff, sizeof(buff));*
Fermeture: *close(fd);*
...
**Question 1** : Selon vous, à quoi correspond le paramètre *myshm* indiqué via
                 l'option *-s* de *shm_writer*? Et *lock*?

*myshm* représente l'emplacement de mémoire partagée utilisé.
*lock* est un sémaphore indiquant l'utilisation de la mémoire partagée.
